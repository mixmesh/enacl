...
%% @end
-spec pwhash(iodata(), binary(), non_neg_integer()) -> binary().
pwhash(Password, Salt, KeyLen) ->
    pwhash(Password, Salt, KeyLen, interactive, interactive).
...
%% limits.
%% @end
-spec pwhash(Password, Salt, KeyLen, Ops, Mem) -> binary()
    when
      Password :: iodata(),
      Salt     :: binary(),
      KeyLen   :: non_neg_integer(),
      Ops      :: pwhash_limit(),
      Mem      :: pwhash_limit().
pwhash(Password, Salt, KeyLen, Ops, Mem) ->
    enacl_nif:crypto_pwhash(Password, Salt, KeyLen, Ops, Mem, default).
...
%% limits.
%% @end
-spec pwhash(Password, Salt, KeyLen, Ops, Mem, Alg) -> binary()
    when
      Password :: iodata(),
      Salt     :: binary(),
      KeyLen   :: non_neg_integer(),
      ...
      Mem      :: pwhash_limit(),
      Alg      :: pwhash_alg().
pwhash(Password, Salt, KeyLen, Ops, Mem, Alg) ->
    enacl_nif:crypto_pwhash(Password, Salt, KeyLen, Ops, Mem, Alg).
