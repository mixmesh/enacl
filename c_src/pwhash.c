...
ERL_NIF_TERM enacl_crypto_pwhash(ErlNifEnv *env, int argc,
  ...
  if ((argc != 6) ||
      ...
      (!enif_get_uint64(env, argv[2], &hlen)) ||
      !(o = enacl_pwhash_opslimit(env, argv[3])) ||
      !(m = enacl_pwhash_memlimit(env, argv[4])) ||
      !(alg = enacl_pwhash_alg(env, argv[5]))) {
    return enif_make_badarg(env);
  }

  // Check limits
  if ((p.size < crypto_pwhash_PASSWD_MIN) ||
      (p.size > crypto_pwhash_PASSWD_MAX) ||
      (hlen < crypto_pwhash_BYTES_MIN) ||
      (hlen > crypto_pwhash_BYTES_MAX) ||
      (o < crypto_pwhash_OPSLIMIT_MIN) ||
